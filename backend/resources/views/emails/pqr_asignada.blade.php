                ...
                <table class="content" width="100%" cellpadding="0" cellspacing="0" role="presentation">
                    ...
                    <tr>
                        <td class="body-content">
                            <h1>¡PQR Asignada para Su Gestión!</h1>

                            <p>Estimado(a) usuario(a) {{ $usuario->name }} {{ $usuario->primer_apellido }},</p>

                            <p>Se le ha asignado la siguiente PQR: {{ $pqr->pqr_codigo }}</p>

                            <p>Por favor, revise los detalles a
                                continuación y proceda con su gestión lo antes posible.</p>
                            ...
                            <div class="pqr-details-box">
                                ...
                                @if (isset($pqr->created_at) && $pqr->created_at)
                                    {{-- Asegurarse de que created_at exista y no sea null --}}
                                    <p><strong>Fecha de Creación:</strong>
                                        {{ Carbon\Carbon::parse($pqr->created_at)->format('d/m/Y H:i') }}</p>
                                @endif
                                @if (isset($pqr->cliente_email) && $pqr->cliente_email)
                                    {{-- Asumiendo que tienes un campo de email del cliente --}}
                                    <p><strong>Correo del Cliente:</strong> {{ $pqr->cliente_email }}</p>
                                @endif
                            </div>

                            <p>Acceda directamente al sistema para revisar todos los detalles de esta PQR y comenzar a trabajar en ella.</p>


                            <hr class="divider">

                            <p>Agradecemos su diligencia y compromiso en la atención de las solicitudes de nuestros usuarios.</p>
