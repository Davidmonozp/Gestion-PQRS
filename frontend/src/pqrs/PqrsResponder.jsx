...
const PqrsResponder = () => {
  ...
  // 1. EFECTO ÚNICO PARA CARGAR LA PQR (Unificado)
  useEffect(() => {
    const fetchDatosPqr = async () => {
      try {
        if (!pqr_codigo) {
        ...
        // Cargamos directamente desde el endpoint de detalle que trae relaciones (autor, adjuntos)
        ...
        setError(
          err.response?.data?.message ||
            "PQRS no encontrada o error de servidor.",
        );
      }
    };
  ...
  // 2. EFECTO PARA CARGAR PLANTILLAS
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!respuesta.trim()) {
      return Swal.fire(
        "Campo vacío",
        "Por favor redacte una respuesta.",
        "warning",
      );
    }

    try {
      // Usamos el servicio registrarRespuesta que ya gestiona el FormData internamente
      ...
      Swal.fire(
        "Error",
        err.message || "Ocurrió un error al registrar la respuesta",
        "error",
      );
    }
  };
  ...
  const crearMarkupConSaltosDeLinea = (texto) => {
    ...
    // Convierte saltos de línea en etiquetas <br /> para renderizado HTML
    ...
    <>
      ...
      <div className="pqrs-res-container">
            ...
            {pqr.registrador_nombre && (
              <div className="solicitante">
                ...
                {pqr.tipo_solicitud === "Tutela" ? (
                  ...
                  <>
                    <p>
                      <strong>Nombre del Juzgado:</strong>{" "}
                      {pqr.registrador_nombre}
                    </p>
                    <p>
                      <strong>Nombre del Juez:</strong>{" "}
                  ...
                  <p>
                    ...
                    {pqr.registrador_nombre} {pqr.registrador_segundo_nombre}{" "}
                    {pqr.registrador_apellido}{" "}
        ...
        {pqr.archivo && pqr.archivo.length > 0 && (
          <div className="archivos-adjuntos" style={{ marginTop: "10px" }}>
            ...
            {pqr.archivo.map((fileItem, index) => {
              ...
              return (
                <div key={`pqr-file-${index}`} style={{ marginBottom: "10px" }}>
        ...
        {pqr &&
          ...
          pqr.clasificaciones?.some(
            (c) => c.nombre === "Solicitudes de tesorería",
        ...
        <form onSubmit={handleSubmit} className="pqrs-res-form">
          ...
          {pqr.respuestas && pqr.respuestas.length > 0 && (
            <div className="preliminary-responses-section">
                      ...
                      (usuario) =>
                        !pqr.respuestas?.some(
                          (r) =>
                            ...
                            (String(r.es_final) === "0" || !r.es_final) && // Validación robusta de preliminar
              ...
              {/* 1. Filtramos con una lógica que acepte booleanos, 0 numérico o "0" string */}
              {pqr.respuestas
                .filter(
                  (r) => String(r.es_final) === "0" || r.es_final === false,
                )
                ...
                .map((respuesta) => (
                  <div key={respuesta.id} className="respuesta-item-card">
                    ...
                    <p>
                      <strong>Fecha:</strong>{" "}
                      {/* Validación de fecha para evitar "Invalid Date" */}
                    ...
                    <p>
                      <strong>Tipo:</strong>{" "}
                      <span className="tag-interna">Respuesta Preliminar</span>
                    </p>
                    ...
                    <div className="respuesta-content-box">
                      ...
                      <p
                        className="recuadro"
                        dangerouslySetInnerHTML={crearMarkupConSaltosDeLinea(
                          respuesta.contenido || "",
                        )}
                      />
                    </div>
                          ...
                          {respuesta.adjuntos.map((adj, idx) => (
                            <li key={idx} className="adjunto-item">
                              <a
                                ...
                                rel="noopener noreferrer"
                              >
                                {adj.original_name || "Archivo adjunto"}
                              </a>
                            </li>
                          ))}
                        </ul>
              ...
              {/* 2. Mensaje de respaldo si el filtro vacía la lista */}
              {pqr.respuestas.filter(
                (r) => String(r.es_final) === "0" || r.es_final === false,
              ).length === 0 && (
                <p className="no-responses-msg">
